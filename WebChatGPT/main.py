        ...
        if response.ok and response.headers.get('content-type')=="text/event-stream; charset=utf-8":
            for chunk in response.iter_lines(decode_unicode=True,):
                ...
                try:
                    ...
                    #print(resp)
                    if not 'message_id' in resp.keys() and resp['message']['status']=='finished_successfully' and resp['message']['end_turn']== True:
...
```
    ...
    def previous_conversations(self, limit:int=10, offset:int=0,order:str="updated",index:int=0, all:str=False) -> list:
...
```
    ...
    def generate_title(self,conversation_id:str,message_id:str) -> dict:
         ...
         resp = self.session.post(
            self.title_generation_endpoint%{
...
if __name__=='__main__':
    import json
    bot = ChatGPT(conversation_index=0)
    while True:
        msg=input('[*]>>')
        resp = bot.chat(msg)
        print(resp)